...
Lots 1a et 1c codés le même soir ; **1b attend le fichier de Christelle**.
...
- **1b — Petit déj** : nouveau Google Sheet à part ; ligne dans Ce dimanche,
  Mes services, rappels ; pas d'onglet. **En attente : fichier et colonnes à
  recevoir de Christelle** (rien de codé, rien de spéculatif).
...
| 1b | **En attente** du Google Sheet de Christelle (identifiant + colonnes). |
